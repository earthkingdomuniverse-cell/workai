...
function generateMockNotifications(count: number = 10): Notification[] {
  ...
  const titles: Record<NotificationType, string[]> = {
    ...
    deal_update: ['Deal funded', 'Work submitted for review', 'Payment released', 'Deal marked complete'],
  ...
  for (let i = 0; i < count; i++) {
    ...
    const now = new Date();
    const createdAt = new Date(now.getTime() - i * 3600000 * (1 + Math.random() * 24)).toISOString();
    ...
    notifications.push({
      ...
      actionUrl: type === 'message' ? `/messages/conv-${i}` : type === 'deal_update' ? `/deals/${i}` : undefined,
      imageUrl: i % 3 === 0 ? `https://api.dicebear.com/7.x/avataaars/svg?seed=user${i}` : undefined,
      createdAt,
    });
  }
